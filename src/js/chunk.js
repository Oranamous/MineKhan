...
import { Generator, sortBiomes } from "./generator.js"
...
class Chunk {
	constructor(x, z, world, glExtensions, gl, glCache, superflat, caves) {
		...
		this.surfaces = []
		this.noiseThresholds = [] // Store certain noise values for use in the different steps of generation
		this.surfaceBiomes = []
	...
	genChunk(superflat) {
		...
		if (superflat) {
			...
			for (let i = 0; i < 16; i++) {
				for (let k = 0; k < 16; k++) {
					//Find biome info
					//biomes is an array of biome objects defined in generator.js, new biomes should be written there. DO NOT EDIT THIS FILE (chunk.js) UNLESS YOU HAVE TO
					temperature = generator.temperature((trueX + i), (trueZ + k));
					moisture = generator.moisture((trueX + i), (trueZ + k));
					elevation = generator.elevation((trueX + i), (trueZ + k));
					tallness = generator.tallness((trueX + i), (trueZ + k));
					biome = generator.getBiome(temperature, moisture, elevation, tallness);
					...
					for (let j = 2; j < 255; j++) {
						noiseVal = noiseProfile.noise((trueX + i) * 0.025, j * 0.0175, (trueZ + k) * 0.025)
						this.noiseThresholds[i * 256 + k * 16 + j] = (elevation * 12 + 68) - (tallness > 0.25 ? j - noiseVal * (tallness - 0.25) * 128 : j)
						if(j > 32){
							if(elevation * 24 + 72 > (tallness > 0.25 ? j - noiseVal * (tallness - 0.25) * 128 : j)){
								this.setBlock(i, j, k, biome.pallette[2]);
								height = j;
							}
							else{
								if(j < 64){
									this.setBlock(i, j, k, generator.defaultFluid);
								}
							}
						}
						else{
							caveNoiseVal = noiseProfile.noise((trueX + i) * 0.01, j * 0.05, (trueZ + k) * 0.01);
							if(sin((j + 16) * 0.05) < caveNoiseVal + 0.54){
								this.setBlock(i, j, k, generator.defaultBlock);
							}
							else{
								if(j < 8){
									this.setBlock(i, j, k, generator.undergroundFluid);
								}
							}
						}
					}
	...
	render(p, global) {
		...
		gl.uniform1f(glCache.uAnimation, this.animation)
	...
	populate(trees) {
		...
		for (let i = 0; i < 16; i++) {
			for (let k = 0; k < 16; k++) {
				for (let j = 1; j < 255; j++) {
					if(!blockData[this.getBlock(i, j, k)].transparent){
						if(blockData[this.getBlock(i, j + 1, k)].transparent){
							this.surfaces[this.surfaces.length] = [i, j, k, 0]
						}
						if(blockData[this.getBlock(i, j - 1, k)].transparent){
							this.surfaces[this.surfaces.length] = [i, j, k, 1]
						}
					}
				}
			}
		}
		let wx = 0, wz = 0, ground = 0, biome, top = 0, rand = 0, place = false, soilDepth = 0, vinesLength = 0
		for (let i = 0; i < 16; i++) {
			for (let k = 0; k < 16; k++) {
				wx = this.x + i
				wz = this.z + k
				...
				//Generate and decorate surfaces
				for(let j = 0; j < this.surfaces.length; j++){
					if(this.surfaces[j][3] === 0 && this.noiseThresholds[i * 256 + k * 16 + this.surfaces[j][1]] < 8){
						soilDepth = floor(random(2, 4));
						this.setBlock(this.surfaces[j][0], this.surfaces[j][1], this.surfaces[j][2], biome.pallette[0]);
						for(let a = 1; a < soilDepth; a++) {
							if(blockData[this.getBlock(this.surfaces[j][0], this.surfaces[j][1] - a, this.surfaces[j][2])].transparent){break;}
							this.setBlock(this.surfaces[j][0], this.surfaces[j][1] - a, this.surfaces[j][2], biome.pallette[1]);
						}
					}
					if(this.surfaces[j][3] === 1){
						vinesLength = random(0, 8);
						if(random(0, 1) < 0.2){
							for(let a = 1; a < vinesLength; a++) {
								if(this.getBlock(this.surfaces[j][0], this.surfaces[j][1] - a, this.surfaces[j][2]) !== blockIds.air){break;}
								this.setBlock(this.surfaces[j][0], this.surfaces[j][1] - a, this.surfaces[j][2], blockIds.leaves);
							}
						}
					}
				}
	...
	genMesh(indexBuffer, bigArray, settings) {
		...
		gl.vertexAttribPointer(glCache.aVertex, 3, gl.FLOAT, false, 40, 0)
		gl.vertexAttribPointer(glCache.aTexture, 2, gl.FLOAT, false, 40, 12)
		gl.vertexAttribPointer(glCache.aShadow, 1, gl.FLOAT, false, 40, 20)
		gl.vertexAttribPointer(glCache.aSkylight, 1, gl.FLOAT, false, 40, 24)
		gl.vertexAttribPointer(glCache.aBlocklight, 1, gl.FLOAT, false, 40, 28)
		gl.vertexAttribPointer(glCache.aTemperature, 1, gl.FLOAT, false, 40, 32)
		gl.vertexAttribPointer(glCache.aMaterial, 1, gl.FLOAT, false, 40, 36)
		glExtensions.vertex_array_object.bindVertexArrayOES(null)
		this.lazy = false
	}
	tick() {
